...
function editTest(w1: string, w2: string, d: number, edit: (w1: string, w2: string) => number) {
  const d1 = edit(w1, w2), d2 = edit(w2, w1), d3 = edit(w1, w1), d4 = edit(w2, w2)
  test(`"${w1}" is ${d} from "${w2}"`, () => expect(d1).toBe(d))
  test(`distance from "${w1}" to "${w2}" = the distance from "${w2}" to "${w1}"`, () => expect(d1).toBe(d2))
