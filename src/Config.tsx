...
function Params({ config }: { config: Config }) {
    ...
    return (<TitleBox title="Parameters" mt={3}>
        ...
        <Grid container alignItems="center" spacing={2}>
            ...
            <Grid item>
                <InfoSpinner flipped={firstInfo} setFlipped={setFirstInfo} fontSize="small" />
            </Grid>
        </Grid>
        <InfoBox shown={firstInfo}>
            The side of flashcards that is shown first. Typically it is easier to start with
            the phrase.
        </InfoBox>
    </TitleBox>)
}
...
function DownloadUpload({ config }: { config: Config }) {
    ...
    return (<TitleBox title="Upload/Download" mt={3}>
        <Grid container alignItems="center" spacing={2}>
            <Grid item xs={4} className={classes.label} container justify="flex-end">
                download
            </Grid>
            <Grid item>
            ...
            <Grid item>
                <InfoSpinner flipped={showDownloadInfo} setFlipped={setShowDownloadInfo} />
            </Grid>
        </Grid>
        <InfoBox shown={showDownloadInfo}>
            Downloading Amanuensis state will give you a JSON file containing
            everything Amanuensis has stored locally. This is is useful if you wish to
            back Amanuensis up or transfer your notes to a different browser or
            machine.
        </InfoBox>
        <Grid container alignItems="center" spacing={2}>
            <Grid item xs={4} className={classes.label} container justify="flex-end">
            ...
            <Grid item>
                <InfoSpinner flipped={showUploadInfo} setFlipped={setShowUploadInfo} />
            </Grid>
        </Grid>
        <InfoBox shown={showUploadInfo}>
            <Box mb={1}>
                Restore Amanuensis from a downloaded file.
            </Box>
            <Box>
                <strong>Note:</strong> restoring Amanuensis from a downloaded JSON file will
    ...
    return (
        <TitleBox title="Statistics" mt={3} style={maybeHide}>
            <Grid container alignItems="center" spacing={2}>
                ...
                <Grid item xs={1}>
                    <InfoSpinner flipped={showMemoryInfo} setFlipped={setShowMemoryInfo} />
                </Grid>
            </Grid>
...
function Clear({ config }: { config: Config }) {
    ...
    return (
        <TitleBox title="Clear Records" mt={3} style={maybeHide}>
            <p className={classes.abstract}>Delete all records of particular types.</p>
            <Grid container alignItems="center" spacing={2}>
                <Grid item xs={4} className={classes.label} container justify="flex-end">
                    everything
                </Grid>
                ...
                <Grid item><InfoSpinner flipped={everythingInfo} setFlipped={setEverythingInfo} /></Grid>
            </Grid>
            <InfoBox shown={everythingInfo}>
                This will delete all information stored by Amanuensis on your computer for this browser.
            </InfoBox>
            <Grid container alignItems="center" spacing={2}>
                <Grid item xs={4} className={classes.label} container justify="flex-end">
                    saved searches
                ...
                <Grid item><InfoSpinner flipped={savedSearchInfo} setFlipped={setSavedSearchInfo} /></Grid>
            </Grid>
            <InfoBox shown={savedSearchInfo}>
                This will delete only the saved search parameters stored by Amanuensis on your computer for this browser.
            </InfoBox>
        </TitleBox>
    )
